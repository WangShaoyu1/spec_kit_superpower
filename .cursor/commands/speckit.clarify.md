...
注意: 此澄清工作流应在调用 `/speckit.design-pd` 之前运行(并完成). 如果用户明确表示他们跳过澄清(例如, 探索性原型), 你可以继续, 但必须警告下游返工风险增加.
...
2. 加载当前规范文件. 使用此分类法执行结构化模糊性和覆盖范围扫描. 对于每个类别, 标记状态: 清晰 / 部分 / 缺失. 生成用于优先级排序的内部覆盖范围图(除非不会提问, 否则不输出原始图).
   ...
   模块与交付物映射(PD 衔接): 
   - 是否存在模块映射表(模块→交付物类型→涉及 FR→PD Key)
   - 所有 FR 是否标注了交付物类型标签([UI:xxx] / [API] / [Infra])
   - UI 类 FR 是否有交互提示(关键 FR)
   - 模块边界是否清晰(有独立 CRUD 闭环的功能应拆为独立模块)
