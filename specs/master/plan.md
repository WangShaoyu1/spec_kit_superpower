...
based_on:
  ...
  - dd/ (7 文档, v2.0)
changelog: |
  2.1: 启动 intent-library 基线重做，补充真实业务链路、证据门禁与显式延期口径
...
**分支**: `master` | **日期**: 2026-03-22 | **规范**: [spec.md](./spec.md)
...
- 本轮按 `pd -> ad -> dd -> plan -> tasks -> code -> verification` 重走 `intent-library`，允许直接修补现有实现。
...
- 任何“已提交/演示成功/占位 UI”都不计入完成；必须以真实 API、真实状态推进和真实验证证据为准。

### Pre-flight 一致性扫描

| 检查项 | 结果 | 说明 |
|--------|------|------|
| PD 页面边界 | WARNING | `intent-library` 实际为 5 页，旧文档误写为 6 页并虚构 `model-download.html` |
| 设计与任务状态口径 | BLOCKER | `tasks-intent-library.md` 声称 41/41 完成，但同文件又保留“Excel 未实现 / UI only / 待确认” |
| LLM 合成反馈策略 | WARNING | 必须同步返回真实结果或明确错误，禁止伪成功文案 |
| 测试消费契约 | WARNING | 单条测试、消息列表、分析结果需按真实包络/分页消费，不能靠前端猜测 |
| 数据集样本展示 | WARNING | `sample_target` 缺失时只能展示真实数量，不能伪造 `0/500` 一类分母 |

**处理原则**:
- `BLOCKER` 未解除前，不得把旧任务状态作为完成依据
- `WARNING` 必须下传到 `tasks`，拆成显式验证任务或延期项
...
1. 建库与数据准备: 创建指令库 → 创建训练/评估数据集 → 维护意图/词槽/实体/相似问
2. 训练闭环: 基于训练集创建模型版本 → 启动训练 → 状态从 `draft` 推进到 `trained`
3. 测试闭环: 在 `test.html` 完成单条测试与批量测试 → 查看真实结果与智能分析
4. 发布闭环: 设置 `testable` / `published` → 下载模型 → 被对话方案发布门禁正确消费

### 显式未完成声明

| 项目 | 当前口径 | 后续要求 |
|------|---------|---------|
| 数据集 Excel 导入/导出 | 可设计，是否落地待本轮任务确认 | 若本轮不做，必须在 `tasks` 与 UI 中显式标记 `Deferred` |
| LLM 合成长任务化/队列化 | 本轮不做 | 当前仅允许同步执行并返回真实结果 |
| `FR-050` 阈值继承展示 | PD 仍为部分覆盖 | 允许本轮先补真实阈值快照与契约验证，UI 继承说明可后续补强 |
...
- **EC-3-3**: 训练/评估数据字段规范、LLM 生成规则、单条/批量测试边界在三份文档中可追溯，且显式包含“不能假成功”约束。
- **EC-3-4**: 形成可直接进入研发的任务拆解，任务状态默认以真实证据为准；历史任务记录不得直接继承为“已完成”。
- **EC-3-5**: 至少定义 1 条端到端核心业务链路和对应验证证据要求，用于后续 smoke/review 对照。

## 版本迭代表

| 版本 | 日期 | 变更摘要 | 来源 |
|---|---|---|---|
| 2.1 | 2026-03-22 | 启动 intent-library 基线重做; 增补 pre-flight、一致性警报、核心业务链路、显式延期项与证据门禁 | `pd-all/pd-intent-library` 重走 |
