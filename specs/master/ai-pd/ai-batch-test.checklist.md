# AI-PD 人工校对清单: batch-test

## 使用方式

- 本清单只覆盖高风险语义点，不要求对 `ai-batch-test.md` 做全文重审
- 校对时必须直接修改 `ai-batch-test.md`
...
- [ ] 页面索引完整，页面数量与 `pd-all/pd-batch-test/` 中 HTML 文件一致
...
|---------|-----------|----------|----------------|-------------|------------------|------------|
| `batch-test.detail` | 对话方案批次详情 | `sub-page` | `detail.html` | `batch-test.index` | 6 | 是 |
| `batch-test.index` | 对话方案批量测试 | `main-page` | `index.html` | `batch-test.detail` | 9 | 是 |
...
- [ ] 若本页 `business_rules / exception_flows` 明显缺失，已直接回写到 `ai-batch-test.md`
