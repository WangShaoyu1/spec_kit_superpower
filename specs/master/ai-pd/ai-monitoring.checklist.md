# AI-PD 人工校对清单: monitoring

## 使用方式

- 本清单只覆盖高风险语义点，不要求对 `ai-monitoring.md` 做全文重审
- 校对时必须直接修改 `ai-monitoring.md`
...
- [ ] 页面索引完整，页面数量与 `pd-all/pd-monitoring/` 中 HTML 文件一致
...
|---------|-----------|----------|----------------|-------------|------------------|------------|
| `monitoring.alert-rules` | 告警规则配置 | `sub-page` | `alert-rules.html` | `monitoring.index` | 5 | 是 |
| `monitoring.device-logs` | 设备日志 | `sub-page` | `device-logs.html` | `monitoring.index` | 2 | 是 |
| `monitoring.index` | 监控仪表盘 | `main-page` | `index.html` | `none` | 5 | 是 |
...
- [ ] 若本页 `business_rules / exception_flows` 明显缺失，已直接回写到 `ai-monitoring.md`
