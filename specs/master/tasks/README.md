...
| 2 | `tasks-intent-library.md` | `pd-intent-library` | P1 | 10 | `pd-user-mgmt` | ✅ |
...
- `tasks-intent-library.md` 已按 `ai-intent-library.md` 完成试验链路重建，并通过定向测试、browser smoke 与 review 收口；旧 tasks 仅保留为参考层
...
### 当前模块模式
intent-library → 单模块试验已完成，`AI-PD -> AD -> DD -> plan -> tasks -> implement -> smoke -> review` 新链路已闭环

## 全局完成定义

- [x] 所有纳入范围的任务均已勾选完成
- [ ] 显式未完成声明已登记且未被误算为完成
- [x] `pd-user-mgmt`、`pd-intent-library`、`pd-knowledge-base`、`pd-dialog-profile` 与 `pd-batch-test` 均有 browser 证据
- [ ] 关键模块性能门槛已冻结并在实现中回读
- [x] 不存在前端假成功提示或未回读的状态/统计
