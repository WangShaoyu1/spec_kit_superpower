...
|---------|-----------|---------------|----------------|------------------|------------|
| `user-mgmt.index` | 用户管理 | `main-page` | `index.html` | 8 | FR-001 |
...
- 创建 PM / tester 账号并直接授予预设角色
- 调整角色后预览能力点变化，确认是否影响发布、测试、监控等关键操作
...
| `CAP-UM-INDEX-006` | 编辑当前项 | `CRUD` | `P1` | 保存后页面主数据回读为最新值 |
| `CAP-UM-INDEX-007` | 删除当前项 | `CRUD` | `P1` | 删除后列表或详情不再显示目标项 |
| `CAP-UM-INDEX-008` | 发布模型 | `STATE` | `P0` | 发布完成后模型状态真实回读更新 |
...
| `ACT-006` | 编辑当前项 | 必要字段合法且权限满足 | 保存后页面主数据回读为最新值 | 必须给出明确错误提示，不允许假成功 | 编辑当前项 |
| `ACT-007` | 删除当前项 | 必要字段合法且权限满足 | 删除后列表或详情不再显示目标项 | 必须给出明确错误提示，不允许假成功 | 删除当前项 |
