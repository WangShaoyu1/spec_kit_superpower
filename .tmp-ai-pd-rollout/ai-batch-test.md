...
|---------|-----------|---------------|----------------|------------------|------------|
| `batch-test.detail` | 对话方案批次详情 | `sub-page` | `detail.html` | 3 | FR-012, FR-013, FR-014 |
| `batch-test.index` | 对话方案批量测试 | `main-page` | `index.html` | 3 | FR-012, FR-013, FR-014 |
...
goal: 团队可以基于统一批次结果决定哪个对话方案更适合进入发布链路
...
- 对多个候选对话方案做统一批量评测，比较整体效果
- 在版本变更后做回归验证，确认是否出现准确率下降或混淆增加
...
| `CAP-BT-DETAIL-002` | 编辑当前项 | `CRUD` | `P1` | 保存后页面主数据回读为最新值 |
| `CAP-BT-DETAIL-003` | 查看智能分析 | `FEEDBACK` | `P1` | 分析结果必须来自真实计算或明确失败原因 |
...
| capability_id | title | type | priority | done_signal |
|---------------|-------|------|----------|-------------|
| `CAP-BT-INDEX-001` | 编辑当前项 | `CRUD` | `P1` | 保存后页面主数据回读为最新值 |
| `CAP-BT-INDEX-002` | 删除当前项 | `CRUD` | `P1` | 删除后列表或详情不再显示目标项 |
| `CAP-BT-INDEX-003` | 查看智能分析 | `FEEDBACK` | `P1` | 分析结果必须来自真实计算或明确失败原因 |
